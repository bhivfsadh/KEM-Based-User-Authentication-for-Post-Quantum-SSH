#!/usr/bin/env bash
# run_supp_ciphertext.sh — Supplementary experiment: ciphertext robustness backend
#
# Parameters are supplied by testScripts/supp_ciphertext_robustness/run.
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "$0")/../.." && pwd)"
SUPP_DIR="$ROOT_DIR/testScripts/supp_ciphertext_robustness"
...
# ================================================================
# ================================================================
# Local decapsulation microbenchmark (Table 2)
# ================================================================
run_local_decaps() {
  ...
  local microbench_src="$SUPP_DIR/microbench_decaps.c"
  local microbench_bin="$SUPP_DIR/microbench_decaps"
  ...
  elif [[ -d "/usr/local/include/oqs" ]]; then
    cflags="-I/usr/local/include -DUSE_LIBOQS"
    libs="-L/usr/local/lib -loqs -lcrypto"
    log_info "liboqs found at /usr/local"
  else
    log_info "liboqs not found; compiling in dummy mode"
  fi
  ...
  if command -v taskset >/dev/null 2>&1; then
    ...
    taskset -c "$DECAPS_CORE" "$microbench_bin" "$DECAPS_WARMUP" "$DECAPS_ITERATIONS" | tee "$RESULT_DIR/raw_runs.csv"
  else
    "$microbench_bin" "$DECAPS_WARMUP" "$DECAPS_ITERATIONS" | tee "$RESULT_DIR/raw_runs.csv"
  fi
...
build_mutation_sshd() {
  ...
  local patch_file="$SUPP_DIR/mutation_server.patch"
  if [[ ! -f "$patch_file" ]]; then
    echo "[ERR] patch not found: $patch_file"
    exit 1
  fi

  # Apply patch if not already applied
  if ! grep -q "KEM_TEST_MUTATION" "$ROOT_DIR/auth2-kem.c" 2>/dev/null; then
    log_info "Applying mutation patch..."
    patch -p1 -d "$ROOT_DIR" < "$patch_file" || {
  ...
  # Rebuild with mutation flag
  local orig_cflags
  orig_cflags="$(grep '^CFLAGS=' "$ROOT_DIR/Makefile" | head -1 | sed 's/CFLAGS=//')"
  cd "$ROOT_DIR"
  make CFLAGS="-DKEM_TEST_MUTATION $orig_cflags" -j4 sshd sshd-session sshd-auth 2>&1 | tail -3
  make -j4 ssh ssh-keygen 2>&1 | tail -2
  cd - >/dev/null

  log_info "Mutation-enabled binaries built"
}
...
case "$MODE" in
  protocol)
    # ---- Source harness (EXACTLY like original working script) ----
    ...
    source "$SUPP_DIR/sshd_test_harness.sh"
    ...
    # ---- Test runner (TOP-LEVEL, like original) ----
    run_single_test() {
...
" 2>/dev/null || echo "NA")"
    ...
    # ---- Analyze ----
    if [[ -x "$SUPP_DIR/analyze_test3.py" ]]; then
      python3 "$SUPP_DIR/analyze_test3.py" protocol "$RAW_CSV" "$SUMMARY_CSV" "$RESULT_DIR/readable.md" 2>/dev/null || true
    fi
    ;;
  local)
    run_local_decaps
  ...
  build)
    ...
    ;;
  *)
    echo "[ERR] unknown mode: $MODE (expected: protocol|local|all|build)"
    exit 1
    ;;
esac
