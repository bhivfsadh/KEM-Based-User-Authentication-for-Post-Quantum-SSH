#!/usr/bin/env python3
# analyze_test1.py — Exp 1 result aggregation and analysis
#
# Read per-round data from summary.csv, compute medians, and output a table
# in testInfo.md §1.1 format.
#
# Usage:
#   python3 analyze_test1.py <summary_csv> [output_md]
...
def generate_markdown_table(agg: dict, raw_agg: dict = None) -> str:
    """Generate Markdown table in testInfo.md §1.1 format (with Attempted/Success/Failed)."""
    lines = []
    lines.append('# Exp 1: Full-SSH Concurrency Throughput and Server Resource Consumption — Results Summary')
    lines.append('')

    # ---- Main table: testInfo §1.1 format + Attempted/Success/Failed + p95 ----
